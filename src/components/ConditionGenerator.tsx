...
let condArr = [{ id: 1, key: "Title", condition: "Equals ( == )", inp: "abc" }];
let keyArr: string[] = ["Title", "Quantity", "Price", "Brand"];
let strConditonArr: string[] = [
  "Equals ( == )",
  "Not Equals ( != )",
  "Contain (%LIKE%)",
  "Not Contain (!%LIKE%)",
];
let intConditionArr: string[] = [
  "Equals ( == )",
  "Not Equals ( != )",
  "Less Than equals ( <= )",
  "Greater than equals ( >= )",
...
const ConditionGenerator = () => {
  let [conditions, setConditions] = useState<defaultType[]>(condArr);
  let [condition, setCondition] = useState<string>("Any Condition");
  let refsArr = useRef<any>([]);
  let keyRefs = useRef<any>([]);
  let condRefs = useRef<any>([]);
  ...
  const addRow = () => {
    if (conditions[conditions.length - 1].inp !== "") {
      let obj = { id: 2, key: "Title", condition: "==", inp: "" };
      conditions.push(obj);
      setConditions([...conditions]);
    } else {
        ...
        return (
          <div className="col-12 d-flex flex-row justify-content-center align-items-center">
            <select
              ...
              className="col-3 p-2 m-1 rounded shadow fs-6 border-3 border-primary"
            >
              {keyArr.map((item: any) => {
            ...
            <select
              ...
              onChange={(e) => selectCondition(e, i)}
              ref={(ref) => (condRefs.current[i] = ref)}
              className="col-3 p-2 m-1 rounded shadow fs-6 border-3 border-primary"
            >
              {arr.map((item: any) => {
                return <option key={item}>{item}</option>;
            ...
            <input
              value={item.inp}
              type={
                item.key === "Title" || item.key === "Brand" ? "text" : "number"
              }
              ref={(ref) => (refsArr.current[i] = ref)}
              onChange={() => inpHandler(i)}
              className="col-3 p-2 m-1 rounded shadow fs-6 border-3 border-primary"
            />
            {i !== 0 ? (
